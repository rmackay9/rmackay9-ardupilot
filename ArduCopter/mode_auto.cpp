...
void ModeAuto::circle_start()
{
    ...
    if (!is_positive(circle.radius_m)) {
        ...
        // a rate of zero gives the panorama no duration, so fall back to the yaw slew limit,
        // which is what set_fixed_yaw_rad() uses for the same case.  One expression feeds both
        // the yaw command and the duration below so the two cannot disagree
        const float rate_rads = is_positive(fabsf(rate_degs)) ? radians(fabsf(rate_degs))
