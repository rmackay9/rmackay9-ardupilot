...
void ModeGuided::run()
{
    ...
    case GuidedSubMode::Guided_TakeOff:
        // run takeoff controller
        takeoff_run();
        break;

    case GuidedSubMode::Guided_WP:
        // run position controller
        pos_control_run();
        break;

    case GuidedSubMode::Guided_Velocity:
        // run velocity controller
        vel_control_run();
        break;

    case GuidedSubMode::Guided_PosVel:
        // run position-velocity controller
        posvel_control_run();
        break;

    case GuidedSubMode::Guided_Angle:
...
{
    guided_mode = GuidedSubMode::Guided_TakeOff;
...
void ModeGuided::pos_control_start()
{
    ...
    guided_mode = GuidedSubMode::Guided_WP;
...
void ModeGuided::vel_control_start()
{
    ...
    guided_mode = GuidedSubMode::Guided_Velocity;
...
{
    // set guided_mode to velocity controller
    guided_mode = GuidedSubMode::Guided_PosVel;
...
{
    return (guided_mode == GuidedSubMode::Guided_TakeOff);
}
...
{
    // set guided_mode to velocity controller
    guided_mode = GuidedSubMode::Guided_Angle;
...
bool ModeGuided::set_destination(const Vector3f& destination, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool terrain_alt)
{
    ...
    if (guided_mode != GuidedSubMode::Guided_WP) {
        pos_control_start();
    }
    ...
    // log target
    copter.Log_Write_GuidedTarget((uint8_t)guided_mode, destination, Vector3f());
    return true;
}
...
bool ModeGuided::set_destination(const Location& dest_loc, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
    ...
    // log target
    copter.Log_Write_GuidedTarget((uint8_t)guided_mode, Vector3f(dest_loc.lat, dest_loc.lng, dest_loc.alt),Vector3f());
    return true;
}
...
void ModeGuided::set_velocity(const Vector3f& velocity, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool log_request)
{
    ...
    if (guided_mode != GuidedSubMode::Guided_Velocity) {
        vel_control_start();
    }
    ...
    // log target
    if (log_request) {
        copter.Log_Write_GuidedTarget((uint8_t)guided_mode, Vector3f(), velocity);
    }
}
...
bool ModeGuided::set_destination_posvel(const Vector3f& destination, const Vector3f& velocity, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
    ...
    // check we are in velocity control mode
    if (guided_mode != GuidedSubMode::Guided_PosVel) {
    ...
    // log target
    copter.Log_Write_GuidedTarget((uint8_t)guided_mode, destination, velocity);
    return true;
}
...
{
    if (mode() == GuidedSubMode::Guided_WP) {
