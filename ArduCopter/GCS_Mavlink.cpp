...
void GCS_MAVLINK_Copter::send_position_target_local_ned()
{
    ...
    const ModeGuided::GuidedSubMode guided_mode = copter.mode_guided.mode();
    Vector3f target_pos;
    Vector3f target_vel;
    uint16_t type_mask;

    if (guided_mode == ModeGuided::GuidedSubMode::Guided_WP) {
        type_mask = 0x0FF8; // ignore everything except position
        target_pos = copter.wp_nav->get_wp_destination() * 0.01f; // convert to metres
    } else if (guided_mode == ModeGuided::GuidedSubMode::Guided_Velocity) {
