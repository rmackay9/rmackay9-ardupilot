...
const float OA_BENDYRULER_LOW_SPEED_SQUARED = 0.04f;    // when ground course is below this speed squared, vehicle's heading will be used (0.04 = 0.2m/s)
...
bool AP_OABendyRuler::update(const Location& current_loc, const Location& destination, const Vector2f &ground_speed_vec, Location &origin_new, Location &destination_new)
{
    ...
    if (ground_speed_vec.length_squared() < OA_BENDYRULER_LOW_SPEED_SQUARED) {
        ...
        ground_course_deg = ground_speed_vec.angle();
    }
    ...
    // search in 5 degree increments around the vehicle alternating left
    ...
    for (uint16_t i = 0; i < (360 / OA_BENDYRULER_BEARING_INC); i++) {
        float bearing_delta = i * OA_BENDYRULER_BEARING_INC * 0.5f;
        if (i & 1) {
        ...
        const float bearing_change_deg = wrap_180(bearing_test - ground_course_deg);

        // don't consider 180 degree turns, as we can't predict which way we will turn
        if (fabsf(bearing_change_deg) > 170) {
            continue;
        }
        ...
        if (margin > _margin_max) {
            ...
            // now check if there is a clear path to the destination
...
bool AP_OABendyRuler::calc_margin_from_circular_fence(const Location &start, const Location &end, float &margin)
{
    ...
    if ((fence_ptr->get_enabled_fences() & AC_FENCE_TYPE_CIRCLE) == 0) {
        return false;
    }

    // calculate start and end point's distance from home
    const Location ahrs_home = AP::ahrs().get_home();
    const float start_dist = ahrs_home.get_distance_NE(start).length();
    const float end_dist = ahrs_home.get_distance_NE(end).length();

    // get circular fence radius
    const float fence_radius = fence_ptr->get_radius();

    // margin is fence radius minus the longer of start or end distance
    margin = fence_radius - MAX(start_dist, end_dist);
    return true;
}
...
bool AP_OABendyRuler::calc_margin_from_polygon_fence(const Location &start, const Location &end, float &margin)
{
    ...
    if (((fence_ptr->get_enabled_fences() & AC_FENCE_TYPE_POLYGON) == 0) || !fence_ptr->is_polygon_valid()) {
        return false;
    }
...
{
    AP_Proximity* prx_ptr = AP_Proximity::get_singleton();
    if (prx_ptr == nullptr) {
        return false;
    }

    // retrieve obstacles from proximity library
    if (!prx_ptr->copy_locations(_prx_locs, ARRAY_SIZE(_prx_locs), _prx_loc_count)) {
        return false;
    }
    ...
    for (uint16_t i=0; i<_prx_loc_count; i++) {
        ...
        if (!margin_found || m < smallest_margin) {
            margin_found = true;
            smallest_margin = m;
        }
    }
