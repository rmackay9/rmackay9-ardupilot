...
#define OA_DIJKSTRA_POLYGON_VISGRAPH_INFINITY_CM 99999999.0f
...
bool AP_OADijkstra::check_polygon_fence_updated() const
{
    ...
    const AC_Fence *fence_ptr = AC_Fence::get_singleton();
    if (fence_ptr == nullptr) {
        return false;
    }
    return (_polyfence_update_ms != fence_ptr->get_boundary_update_ms());
}
...
bool AP_OADijkstra::create_polygon_fence_with_margin(float margin_cm)
{
    ...
    uint16_t num_points;
    const Vector2f* boundary = fence_ptr->get_boundary_points(num_points);
    ...
    for (uint8_t i=0; i<num_points; i++) {
        ...
        Vector2f after_pt = boundary[i+1] - boundary[i];
        ...
        if (Polygon_outside(_polyfence_pts[i], boundary, num_points)) {
            ...
            if (Polygon_outside(_polyfence_pts[i], boundary, num_points)) {
                ...
                // this can happen if fence lines are closer than margin_m
                return false;
            }
        }

        _polyfence_numpoints++;
    }

    // last point is same as first point
    _polyfence_pts[_polyfence_numpoints] = _polyfence_pts[0];
    _polyfence_numpoints++;

    // record fence update time so we don't process this exact fence again
    _polyfence_update_ms = fence_ptr->get_boundary_update_ms();
        ...
        for (uint8_t j=i+1; j<_polyfence_numpoints-1; j++) {
            ...
            if (!Polygon_intersects(boundary, num_points, start1, end1, intersection)) {
                ...
                _polyfence_visgraph.add_item({AP_OAVisGraph::OATYPE_FENCE_POINT, i},
                                              ...
                                              safe_sqrt(sq(_polyfence_pts[i].x - _polyfence_pts[j].x) + sq(_polyfence_pts[i].y - _polyfence_pts[j].y)));
            }
...
// requires create_polygon_fence_with_margin to have been run
// returns true on success
bool AP_OADijkstra::update_visgraph(AP_OAVisGraph& visgraph, const AP_OAVisGraph::OAItemID& oaid, const Vector2f &position, bool add_extra_point, Vector2f extra_point)
{
    // exit immediately if no polygon fence (with margin)
    if (_polyfence_numpoints == 0) {
        return false;
    ...
    for (uint8_t i=0; i<_polyfence_numpoints-1; i++) {
        ...
        if (!Polygon_intersects(boundary, num_points, position, _polyfence_pts[i], intersection)) {
            // line segment does not intersect with original fence so add to visgraph
            visgraph.add_item(oaid, {AP_OAVisGraph::OATYPE_FENCE_POINT, i}, safe_sqrt(sq(position.x - _polyfence_pts[i].x) + sq(position.y - _polyfence_pts[i].y)));
        }
        // ToDo: store infinity when there is no clear path between points to allow faster search later
    }
    ...
    if (add_extra_point) {
        Vector2f intersection;
        if (!Polygon_intersects(boundary, num_points, position, extra_point, intersection)) {
            visgraph.add_item(oaid, {AP_OAVisGraph::OATYPE_DESTINATION, 0}, safe_sqrt(sq(position.x - extra_point.x) + sq(position.y - extra_point.y)));
        }
    }
...
void AP_OADijkstra::update_visible_node_distances(node_index curr_node_idx)
{
    ...
    const AP_OAVisGraph* visgraph_ptrs[] = {&_polyfence_visgraph, &_destination_visgraph};
    for (uint8_t v=0; v<ARRAY_SIZE(visgraph_ptrs); v++) {

        // skip if empty
        const AP_OAVisGraph &curr_visgraph = *visgraph_ptrs[v];
...
bool AP_OADijkstra::find_node_from_id(const AP_OAVisGraph::OAItemID &id, node_index &node_idx) const
{
    ...
    if ((id.id_type == AP_OAVisGraph::OATYPE_SOURCE) && (_short_path_data_numpoints > 0)) {
        node_idx = 0;
        return true;
    }

    // destination is always the 2nd node
    if ((id.id_type == AP_OAVisGraph::OATYPE_DESTINATION) && (_short_path_data_numpoints > 1)) {
        node_idx = 1;
        return true;
    }

    // must be a fence node which start from 3rd node
    if ((id.id_type == AP_OAVisGraph::OATYPE_FENCE_POINT) && (_short_path_data_numpoints > id.id_num + 2)) {
...
bool AP_OADijkstra::find_closest_node_idx(node_index &node_idx) const
{
    ...
    float lowest_dist = 0.0f;
    bool lowest_found = false;

    // scan through all nodes looking for closest
    for (uint16_t i=0; i<_short_path_data_numpoints; i++) {
        const ShortPathNode &node = _short_path_data[i];
        if (!node.visited && (!lowest_found || node.distance_cm < lowest_dist)) {
            lowest_found = true;
            lowest_idx = i;
...
bool AP_OADijkstra::calc_shortest_path(const Location &origin, const Location &destination)
{
    ...
    // clear short_path_data array
    _short_path_data_numpoints = 0;

    // add origin and destination (node_type, id, visited, distance, distance_from_idx) to short_path_data array
    _short_path_data[_short_path_data_numpoints] = {{AP_OAVisGraph::OATYPE_SOURCE, 0}, false, 0, 0};
    _short_path_data_numpoints++;
    _short_path_data[_short_path_data_numpoints] = {{AP_OAVisGraph::OATYPE_DESTINATION, 0}, false, OA_DIJKSTRA_POLYGON_VISGRAPH_INFINITY_CM, OA_DIJKSTRA_POLYGON_SHORTPATH_NOTSET_IDX};
    _short_path_data_numpoints++;

    // add fence points to short_path_data array (node_type, id, visited, distance, distance_from_idx)
    // skip last fence point because it is the same as the first
    for (uint8_t i=0; i<_polyfence_numpoints-1; i++) {
        _short_path_data[_short_path_data_numpoints] = {{AP_OAVisGraph::OATYPE_FENCE_POINT, i}, false, OA_DIJKSTRA_POLYGON_VISGRAPH_INFINITY_CM, OA_DIJKSTRA_POLYGON_SHORTPATH_NOTSET_IDX};
        _short_path_data_numpoints++;
    }
    ...
    bool success = false;
    {
        ...
        while (!complete) {
            ...
            if ((_path_numpoints >= OA_DIJKSTRA_POLYGON_SHORTPATH_PTS) ||
                ...
                (_short_path_data[nidx].distance_cm >= OA_DIJKSTRA_POLYGON_VISGRAPH_INFINITY_CM)) {
                complete = true;
            } else {
...
// print out all node data for debug purposes
void AP_OADijkstra::print_all_nodes()
{
    ::printf("Nodes %d -----\n",(int)_short_path_data_numpoints);
    for (uint8_t i=0; i<_short_path_data_numpoints; i++) {
        ShortPathNode& curr_node = _short_path_data[i];
        ::printf("%d: id:%d-%d v:%d dist:%4.1f from:%d\n",
                (int)i,
                (int)curr_node.id.id_type,
                (int)curr_node.id.id_num,
                (int)curr_node.visited,
                (double)curr_node.distance_cm,
                (int)curr_node.distance_from_idx);
    }
}
...
// return number of points in shortest path
uint8_t AP_OADijkstra::shortest_path_numpoints()
{
    return _path_numpoints;
}

// print shortest path for debug purposes
void AP_OADijkstra::print_shortest_path()
{
    uint8_t path_points = shortest_path_numpoints();
    if (path_points == 0) {
        return;
    }

    ::printf("Shortest Path %d points ----\n",(int)path_points);
    for (int16_t j=0; j<path_points; j++) {
        Vector2f pos;
        if (get_shortest_path_point(j, pos)) {
            ::printf("path%d x:%4.1f y:%4.1f\n",(int)j, (double)pos.x, (double)pos.y);
        } else {
            ::printf("failed to get path point %d\n",(int)j);
        }
    }
    ::printf("----\n");
}
