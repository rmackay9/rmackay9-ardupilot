...
// Closest point between point(p) to a point contained in the line segment defined by w1,w2
...
{
    // direction vectors
    Vector3f line1 = seg1_end - seg1_start;
    Vector3f line2 = seg2_end - seg2_start;

    Vector3f diff = seg1_start - seg2_start;
    ...
    Vector3f closest_line_segment = diff + (line1*sc) - (line2*tc);
    float len = closest_line_segment.length();
...
template float Vector3<float>::segment_to_segment_dist(const Vector3<float>& seg1_start, const Vector3<float>& seg1_end, const Vector3<float>& seg2_start, const Vector3<float>& seg2_end, Vector3<float>& intersection);
