...
void AP_OpticalFlow_Calibrator::start()
{
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "FlowCal: Started");
}
...
void AP_OpticalFlow_Calibrator::stop()
{
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "FlowCal: Stopped");
}
    ...
    if (_cal_state == CalState::RUNNING) {
        ...
        if (AP::ahrs().getOptFlowSample(timestamp_ms, flow_rate, body_rate, los_pred)) {
            ...
            if (sample_buffers_full()) {
                ...
                gcs().send_text(MAV_SEVERITY_INFO, "FlowCal: samples collected");
            }
        }
...
bool AP_OpticalFlow_Calibrator::calc_scalars(uint8_t axis, float& scalar, float& fitness)
{
    ...
    if (num_samples == 0) {
        gcs().send_text(MAV_SEVERITY_INFO, "%s failed because no samples", prefix_str);
        return false;
    }
    ...
    if (is_zero(total_abs_residual)) {
        scalar = 1.0;
        fitness = 0;
        gcs().send_text(MAV_SEVERITY_INFO, "%s perfect scaling of 1.0", prefix_str);
        return true;
    }
    ...
    for (uint8_t i = 0; i < num_samples; i++) {
        ...
        if (!calc_sample_best_scalar(samplei, sample_best_scalar)) {
            ...
            gcs().send_text(MAV_SEVERITY_INFO, "%s failed because of zero flow rate", prefix_str);
    ...
    if (best_scalar_total < AP_OPTICALFLOW_CAL_SCALE_MIN) {
        gcs().send_text(MAV_SEVERITY_INFO, "%s scalar:%4.3f too low (<%3.1f)", prefix_str, (double)best_scalar_total, (double)AP_OPTICALFLOW_CAL_SCALE_MIN);
        return false;
    }
    if (best_scalar_total > AP_OPTICALFLOW_CAL_SCALE_MAX) {
        gcs().send_text(MAV_SEVERITY_INFO, "%s scalar:%4.3f too high (>%3.1f)", prefix_str, (double)best_scalar_total, (double)AP_OPTICALFLOW_CAL_SCALE_MAX);
        return false;
    }
    ...
    if (fitness_new > AP_OPTICALFLOW_CAL_FITNESS_THRESH) {
        gcs().send_text(MAV_SEVERITY_INFO, "%s scalar:%4.3f fit:%4.3f too high (>%3.1f)", prefix_str, (double)scalar, (double)fitness_new, (double)AP_OPTICALFLOW_CAL_FITNESS_THRESH);
        return false;
    }
    ...
    if (fitness_new <= fitness_orig) {
        ...
        fitness = fitness_new;
        gcs().send_text(MAV_SEVERITY_INFO, "%s scalar:%4.3f fit:%4.1f", prefix_str, (double)scalar, (double)fitness);
        return true;
    }
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "%s failed! new scalar/fit:%4.3f/%4.3f old:%4.1f/%4.1f", prefix_str, (double)best_scalar_total, (double)fitness_new, (double)scalar, (double)fitness_orig);
    return false;
}
...
// log all samples
void AP_OpticalFlow_Calibrator::log_samples()
{
    uint8_t num_samples_min = MIN(_cal_data[0].num_samples, _cal_data[1].num_samples);
    for (uint8_t i = 0; i < num_samples_min; i++) {
        // @LoggerMessage: OFCA
        // @Description: Optical Flow Calibration debug
        // @Field: TimeUS: Time since system startup
        // @Field: Num: sample number
        // @Field: FRateX: Flow rate (X-Axis)
        // @Field: BRateX: Body rate (X-Axis)
        // @Field: LPredX: Los pred (X-Axis)
        // @Field: FRateY: Flow rate (Y-Axis)
        // @Field: BRateY: Body rate (Y-Axis)
        // @Field: LPredY: Los pred (Y-Axis)

        AP::logger().Write("OFCA", "TimeUS,Num,FRateX,BRateX,LPredX,FRateY,BRateY,LPredY", "QBffffff",
            AP_HAL::micros64(),
            (unsigned)i,
            (double)_cal_data[0].samples[i].flow_rate,
            (double)_cal_data[0].samples[i].body_rate,
            (double)_cal_data[0].samples[i].los_pred,
            (double)_cal_data[1].samples[i].flow_rate,
            (double)_cal_data[1].samples[i].body_rate,
            (double)_cal_data[1].samples[i].los_pred
            );
    }
}
