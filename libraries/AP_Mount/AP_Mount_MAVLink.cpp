...
{
    // check for mavlink gimbal protcol
    AP_SerialManager::serial_state mavgimbal_serial;
    if (serial_manager.find_serial(AP_SerialManager::SerialProtocol_MAVLink2, mavgimbal_serial)) {
        // retrieve the mavlink comm from the serial port
        if (get_comm_from_uart(mavgimbal_serial.uart, _chan)) {
            _initialised = true;
        }
    }
}
