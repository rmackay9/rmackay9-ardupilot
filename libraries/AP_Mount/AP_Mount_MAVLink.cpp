...
#define AP_MOUNT_MAVLINK_RESEND_MS  1000    // resend angle targets to gimbal once per second
...
void AP_Mount_MAVLink::update()
{
    ...
    if (resend_now || ((AP_HAL::millis() - _last_send) > AP_MOUNT_MAVLINK_RESEND_MS)) {
...
void AP_Mount_MAVLink::find_gimbal()
{
    ...
    if (AP_HAL::millis() > AP_MOUNT_MAVLINK_SEARCH_MS) {
        return;
    }
