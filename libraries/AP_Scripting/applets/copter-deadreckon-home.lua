...
local wp_speed_up = Parameter("WP_SPEED_UP")           -- maximum climb rate from WP_SPEED_UP
local wp_accel_z = Parameter("WP_ACCEL_Z")             -- maximum vertical acceleration from WP_ACCEL_Z
    ...
    if (fly_alt_min:get() > 0) then
      ...
      if curr_alt_below_home then
        ...
        if target_alt_above_vehicle > 0 then
          -- climb at up to 1m/s towards target above vehicle.  climb rate change is limited by WP_ACCEL_Z
          local climb_rate_chg_max = interval_ms * 0.001 * (wp_accel_z:get() * 0.01)
          climb_rate = math.min(target_alt_above_vehicle * 0.1, wp_speed_up:get() * 0.01, climb_rate + climb_rate_chg_max)
        end
      end
    end
