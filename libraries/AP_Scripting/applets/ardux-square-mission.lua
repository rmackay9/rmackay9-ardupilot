...
--   Set AXMI_WP1_DIST to the distance from home to the first waypoint
--   Set AXMI_WP2_DIST to the distance from WP1 to WP2 (length)
--   Set AXMI_WP3_DIST to the distance from WP2 to WP3 (width)
--
...
local AXMI_ALT = bind_add_param("ALT", 1, 3)                -- mission altitude.  default is 3m
local AXMI_WP1_DIST = bind_add_param("WP1_DIST", 2, 5)      -- distance from takeoff to 1st waypoint.  default is 5m
local AXMI_WP2_DIST = bind_add_param("WP2_DIST", 3, 8)      -- distance from 1st waypoint to 2nd.  default is 8m
...
function update()
  ...
  if arming:is_armed() then
    gcs:send_text(MAV_SEVERITY_ERROR, "AXMI: please disarm vehicle")
    return update, UPDATE_INTERVAL_MS
  end
  ...
  if not curr_loc then
    gcs:send_text(MAV_SEVERITY_ERROR, "AXMI: need location")
    return update, UPDATE_INTERVAL_MS
  end

  -- clear mission
  if not mission:clear() then
    gcs:send_text(MAV_SEVERITY_ERROR, "AXMI: failed to clear mission")
    return update, UPDATE_INTERVAL_MS
  end
  ...
  gcs:send_text(MAV_SEVERITY_INFO, "AXMI: wrote mission")
