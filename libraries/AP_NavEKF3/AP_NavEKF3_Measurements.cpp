...
void NavEKF3_core::readRangeFinderUp()
{
    ...
    if (lastRngUpMeasTime_ms == last_reading_ms) {
        //gcs().send_text(MAV_SEVERITY_CRITICAL, "readRangeFinderUp: duplicate timestamp %lu", (unsigned long)last_reading_ms);
        return;
    }

    // limit update rate to maximum allowed by data buffers
    if ((last_reading_ms - lastRngUpMeasTime_ms) < frontend->sensorIntervalMin_ms) {
        //gcs().send_text(MAV_SEVERITY_CRITICAL, "readRangeFinderUp: too fast STim:%ld LRTim:%ld", (unsigned long)last_reading_ms, (unsigned long)lastRngUpMeasTime_ms);
        return;
    }
