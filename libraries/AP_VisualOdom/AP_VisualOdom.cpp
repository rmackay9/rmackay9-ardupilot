...
const AP_Param::GroupInfo AP_VisualOdom::var_info[] = {
    ...
    // @Description: Visual odometry will only be sent to EKF if over this value.  Ignored if 0
    // @Units: %
    // @Range: 0 100
    // @User: Advanced
...
uint8_t AP_VisualOdom::quality() const
{
    if (_driver == nullptr) {
        return 0;
    }
...
void AP_VisualOdom::handle_pose_estimate(uint64_t remote_time_us, uint32_t time_ms, float x, float y, float z, float roll, float pitch, float yaw, float posErr, float angErr, uint8_t reset_counter, uint8_t quality)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
...
// general purpose method to consume position estimate data and send to EKF
// quality of 0 means unknown, 1 is worst, 100 is best
void AP_VisualOdom::handle_pose_estimate(uint64_t remote_time_us, uint32_t time_ms, float x, float y, float z, const Quaternion &attitude, float posErr, float angErr, uint8_t reset_counter, uint8_t quality)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
...
// quality of 0 means unknown, 1 is worst, 100 is best
void AP_VisualOdom::handle_vision_speed_estimate(uint64_t remote_time_us, uint32_t time_ms, const Vector3f &vel, uint8_t reset_counter, uint8_t quality)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
