...
public:
    ...
    // return quality threshold as a value from 0 to 100
    uint8_t get_quality_min() const { return _quality_min; }
    ...
    void handle_pose_estimate(uint64_t remote_time_us, uint32_t time_ms, float x, float y, float z, float roll, float pitch, float yaw, float posErr, float angErr, uint8_t reset_counter, uint8_t quality);
    void handle_pose_estimate(uint64_t remote_time_us, uint32_t time_ms, float x, float y, float z, const Quaternion &attitude, float posErr, float angErr, uint8_t reset_counter, uint8_t quality);
    
    // general purpose methods to consume velocity estimate data and send to EKF
    // velocity in NED meters per second
    // quality of 0 means unknown, 1 is worst, 100 is best
    void handle_vision_speed_estimate(uint64_t remote_time_us, uint32_t time_ms, const Vector3f &vel, uint8_t reset_counter, uint8_t quality);
