        ...
        if (state[i].uart != nullptr) {
            switch (state[i].protocol) {
                ...
                case SerialProtocol_ESCTelemetry:
                    ...
                    state[i].uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                    break;
                case AP_SERIALMANAGER_HYBRID_BAUD:
                	state[i].baud = AP_SERIALMANAGER_HYBRID_BAUD / 1000;   // update baud param in case user looks at it
					state[i].uart->begin(map_baudrate(state[i].baud),
							AP_SERIALMANAGER_HYBRID_BUFSIZE_RX,
							AP_SERIALMANAGER_HYBRID_BUFSIZE_TX);
                	break;
            }
        }
    }
