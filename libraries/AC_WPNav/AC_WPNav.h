...
public:
    ...
    // Derived from the WP_SPEED parameter.
    ...
    // Derived from the WP_SPEED_UP parameter.
    ...
    // Derived from the WP_SPEED_DN parameter. Always positive.
    ...
    // Derived from the WP_ACCEL_Z parameter. Always positive.
    ...
    // Derived from the WP_ACCEL parameter. Falls back to a default if unset.
    ...
    // Derived from WP_ACCEL_C or defaults to 2x WP_ACCEL if unset.
...
protected:
    ...
    // WP_SPEED param change checker
    bool _check_wp_speed_change;    // true if WP_SPEED should be monitored for changes during flight
    float _last_wp_speed_ms;        // last recorded WP_SPEED value (m/s) for change detection
    float _last_wp_speed_up_ms;     // last recorded WP_SPEED_UP value (m/s)
    float _last_wp_speed_down_ms;   // last recorded WP_SPEED_DN value (m/s)
