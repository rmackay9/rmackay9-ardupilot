                                                       ...
                                                       mavlink_mission_item_int_t &ret_packet)
{
    ...
    if (!fence.polyfence().get_item(packet.seq, fenceitem)) {
...
static MAV_MISSION_RESULT convert_MISSION_ITEM_INT_to_AC_PolyFenceItem(const mavlink_mission_item_int_t &mission_item_int, AC_PolyFenceItem &ret)
{
    if (mission_item_int.frame != MAV_FRAME_GLOBAL &&
        ...
        gcs().send_text(MAV_SEVERITY_WARNING, "Frame must be MAV_FRAME_GLOBAL{,_INT} not %u", mission_item_int.frame);
        return MAV_MISSION_ERROR;
    }
...
MAV_MISSION_RESULT MissionItemProtocol_Fence::complete(const GCS_MAVLINK &_link)
{
    if (_updated_mask != nullptr) {
        ...
        for (uint16_t i=0; i<_new_items_count; i++) {
            if (!(_updated_mask[i/8] & (1U<<(i%8)))) {
                if (!fence.polyfence().get_item(i, _new_items[i])) {
                    _link.send_text(MAV_SEVERITY_INFO, "item replacement error");
                    return MAV_MISSION_ERROR;
                }
            }
        }
    ...
    bool success = fence.polyfence().write_fence(_new_items, _new_items_count);
    if (!success) {
        _link.send_text(MAV_SEVERITY_INFO, "Fence points error");
        return MAV_MISSION_ERROR;
    }

    _link.send_text(MAV_SEVERITY_INFO, "Fence points received");
...
{
    return fence.polyfence().max_items();
}
...
{
    return fence.polyfence().write_fence(nullptr, 0);
}
...
MAV_MISSION_RESULT MissionItemProtocol_Fence::allocate_receive_resources(const uint16_t count)
{
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "Allocating %u bytes for fence upload", allocation_size);
